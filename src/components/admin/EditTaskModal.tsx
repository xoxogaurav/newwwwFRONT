...
export default function EditTaskModal({
  ...
  const [formData, setFormData] = useState({
    ...
    time_in_seconds: task?.time_in_seconds
      ? task.time_in_seconds.toString()
      : '0',
    steps: task?.steps || [],
    approval_type: task?.approval_type || 'manual',
    allowed_countries: task?.allowed_countries || ['IN'],
    hourly_limit: task?.hourly_limit?.toString() || '0',
    daily_limit: task?.daily_limit?.toString() || '0',
    is_active: task?.is_active !== false,
    one_off: task?.one_off !== false,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const taskData = {
        ...
        allowed_countries: formData.allowed_countries.map((code) =>
          code.toUpperCase()
        ),
      ...
      const updatedTask = await AdminService.updateTask(task.id, taskData);
